---
layout: page
icon: fas fa-info-circle
order: 4
---

# 关于我 | About Me

## 简介 | Introduction

你好，我是 Pingo，一名专注于流媒体技术的软件工程师。我热衷于探索和分享音视频技术、WebRTC、直播和视频会议等领域的解决方案。

Hello, I'm Pingo, a software engineer specializing in streaming media technology. I'm passionate about exploring and sharing solutions in audio/video technology, WebRTC, live streaming, and video conferencing systems.

## 专业领域 | Expertise

### 🎯 技术方向 | Technical Focus
- 流媒体技术 | Streaming Media Technology
- WebRTC 技术栈 | WebRTC Stack
- 实时音视频通信 | Real-time Audio/Video Communication
- 直播技术 | Live Streaming Technology
- 视频会议系统 | Video Conferencing Systems

### 💻 技术栈 | Tech Stack
- 编程语言 | Programming Languages
  - C/C++, Go, Python
- 流媒体协议 | Streaming Protocols
  - RTMP, HLS, WebRTC, RTP/RTCP
- 音视频编解码 | Audio/Video Codecs
  - H.264/H.265, AAC, Opus
- 网络传输 | Network Transport
  - TCP/UDP, QUIC
- 开源项目 | Open Source Projects
  - FFmpeg, GStreamer, WebRTC

## 🌟 精选项目 | Featured Projects

<div align="left">
  <a href="https://github.com/pingostack/pingos">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=im-pingo&repo=pingos&theme=radical" alt="PingOS Repository Card" />
  </a>
  <a href="https://github.com/pingostack/livhub">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=pingostack&repo=livhub&theme=radical" alt="Livhub Repository Card" />
  </a>
</div>

### 📺 PingOS
基于 nginx-rtmp 模块的流媒体服务器，支持 RTMP、HTTP-FLV、HLS 等协议，适用于直播和点播场景。

A streaming media server based on nginx-rtmp module, supporting RTMP, HTTP-FLV, HLS, and other protocols for live streaming and VOD scenarios.

### 🚀 Livhub
新一代流媒体服务器，采用 Go 语言开发，支持 WebRTC、RTMP、HLS 等多种协议，提供高性能、低延迟的实时通信解决方案。

Next-generation streaming media server developed in Go, supporting WebRTC, RTMP, HLS, and other protocols, offering high-performance, low-latency real-time communication solutions.

## 📊 GitHub 统计 | GitHub Stats

<div align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=im-pingo&show_icons=true&theme=radical" alt="GitHub Stats" />
</div>

## 📫 联系方式 | Contact

- GitHub: [@im-pingo](https://github.com/im-pingo)
- Email: cczjp89@gmail.com

欢迎交流和讨论任何关于流媒体技术的话题！

Feel free to reach out for any discussions about streaming media technology!

---

<div align="left">
  <img src="https://komarev.com/ghpvc/?username=im-pingo&color=blueviolet" alt="Profile views" />
</div>
